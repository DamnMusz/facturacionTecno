...
app.controller('MainController',
    ...
    $scope.lastMonth = function () {
        var auxM = $scope.padding(((new Date()).getMonth()), 2);
        var res = (auxM != '00') ? auxM : '12';
        res += "/";

        var auxY = ((new Date()).getFullYear());
        if (auxM == '00')
            auxY--;
        return res + auxY;
    }
    ...
    $scope.porcentaje_10 = function (porcion, total) {
        return (porcion / total) * 1000;
    }
    ...
    $scope.sumaColumna = function (matrix, column) {
        var sum = 0;
