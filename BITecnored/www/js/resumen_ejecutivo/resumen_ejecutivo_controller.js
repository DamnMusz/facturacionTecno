app.controller('ResumenEjecutivoController', function ($scope, $compile, ResumenEjecutivoService) {
    ...
    $scope.sumatoria_inspeccionadas_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].inspeccionadas;
        }
        return s;
    }

    $scope.sumatoria_solicitadas_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].solicitadas;
        }
        return s;
    }

    $scope.promedio_fotos_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].prom_fotos;
        }
        return s / $scope.resumen_por_aseguradora.length;
    }

    $scope.promedio_mismo_dia_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].mismo_dia;
        }
        return s / $scope.resumen_por_aseguradora.length;
    }

    $scope.promedio_1_dia_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].en24hs;
        }
        return s / $scope.resumen_por_aseguradora.length;
    }

    $scope.promedio_2_dias_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].en48hs;
        }
        return s / $scope.resumen_por_aseguradora.length;
    }

    $scope.promedio_realizadas_todas = function () {
        var s = 0;
        for (i = 0; i < $scope.resumen_por_aseguradora.length; i++) {
            s += $scope.resumen_por_aseguradora[i].porc_realizadas;
        }
        return s / $scope.resumen_por_aseguradora.length;
    }
    ...
    $scope.propertyName = 'inspeccionadas';
    $scope.reverse = true;
        ...
        if ($scope.filtro.periodo != 'undefined' && $scope.filtro.periodo != '') {
            ...
            ResumenEjecutivoService.getTiempoGestionPorAseguradora($scope.filtro.periodo).then(function (res) {
                ResumenEjecutivoService.getRealizadasPorAseguradora($scope.filtro.periodo).then(function (res_realizadas) {
                    ...
                    for (var i = 0; i < realiz_keys.length; i++) {
                        for (var j = 0; j < $scope.resumen_por_aseguradora.length; j++) {
                            if ($scope.resumen_por_aseguradora[j].aseguradora_alias == realiz_keys[i]) {
                                ...
                                $scope.resumen_por_aseguradora[j].porc_realizadas = $scope.porcentaje_10(res_realizadas.data[realiz_keys[i]].realiz_count, res_realizadas.data[realiz_keys[i]].total_count);
                            }
                        }
                    }
                    ...
                    $scope.total_inspeccionadas = $scope.sumatoria_inspeccionadas_todas();
                    done_loading();
                });
            });
        }
